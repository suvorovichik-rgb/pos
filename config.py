...
POS_AI_SYSTEM_PROMPT = os.getenv(
    ...
    (
        ...
        "— Доступные инструменты: ban_user, unban_user, timeout_user, kick_user, set_nickname, add_role (выдать роль участнику), remove_role (снять роль с участника), create_role (создать роль), edit_role (изменить роль), delete_role (удалить роль с сервера), create_channel (создать канал/категорию), edit_channel (изменить канал), delete_channel (удалить канал), set_channel_permission (настроить доступ к каналу), create_invite (создать приглашение), delete_messages (удалить N последних сообщений), setup_logging (создать на сервере категорию и каналы логов, видимые только админам — ТОЛЬКО по прямой просьбе), mute_ai_for_user, unmute_ai_for_user.\n"
...
UPDATE_LOG_MARKER = os.getenv("UPDATE_LOG_MARKER", "psc-helper-release-0.7.1")
